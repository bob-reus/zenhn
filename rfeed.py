...
class Host(Serializable):
    ...
    def add_extension(self, extension):
        ...
        if not isinstance(extension, Serializable):
            raise TypeError(
                "The provided extension should be a subclass of the Serializable class"
            )
...
class Guid(Serializable):
    ...
    def publish(self, handler):
        ...
        self._write_element(
            "guid", self.guid, {"isPermaLink": "true" if self.isPermaLink else "false"}
        )
...
class iTunes(Extension):
    ...
    def __init__(
        ...
        self.block = (
            True
            if (isinstance(block, basestring) and block.lower() == "yes")
            else block
        )
        self.image = image
        self.explicit = (
            True
            if (isinstance(explicit, basestring) and explicit.lower() == "yes")
            else explicit
        )
        self.complete = (
            True
            if (isinstance(complete, basestring) and complete.lower() == "yes")
            else complete
        )
        self.owner = owner
    ...
    def publish(self, handler):
        ...
        if self.explicit is not None:
            self._write_element(
                "itunes:explicit", "yes" if self.explicit is True else "clean"
            )

        if self.complete is not None:
            self._write_element(
                "itunes:complete", "yes" if self.complete is True else "no"
            )
...
class iTunesItem(Serializable):
    ...
    def __init__(
        ...
        self.is_closed_captioned = (
            True
            if (
                isinstance(is_closed_captioned, basestring)
                and is_closed_captioned.lower() == "yes"
            )
            else is_closed_captioned
        )
        self.order = order
        self.subtitle = subtitle
        self.summary = summary
    ...
    def publish(self, handler):
        ...
        if self.is_closed_captioned is not None:
            self._write_element(
                "itunes:is_closed_captioned",
                "yes" if self.is_closed_captioned is True else "no",
            )
        ...
        for extension in self.extensions:
            if isinstance(extension, Extension):
                ...
                if namespace is not None:
                    attributes = dict(
                        itertools.chain(attributes.items(), namespace.items())
                    )
